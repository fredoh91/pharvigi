...
final class CreationCasController_old extends AbstractController
{
    #[Route('/creation_cas_old', name: 'app_cm_creation_cas_old')]
        ...
        ): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($FicWordRecueilCM) {
                ...
                // On crée l'objet CM pré-rempli mais on ne le persiste pas encore
                $cm = $importService->CreationCasCM($ficRec, $mainData, $eiDataRows, $medicDataRows, $requetesMeddraService);

                // On peut maintenant créer le formulaire final (par ex: CMType) avec cet objet
                // $formFinal = $this->createForm(CMType::class, $cm);
                
                // Pour l'instant, on render une vue qui affichera ce formulaire
                // return $this->render('cm/creation_cas/reviser_cas.html.twig', [
